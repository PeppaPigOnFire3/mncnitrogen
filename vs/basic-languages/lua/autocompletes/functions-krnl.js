define(["require", "exports"], function (require, exports) {
	...
	exports.autocompletes = {
		"Function": {
			...
			"teleporttoplr()": {
				"insertText": "teleporttoplr(\"${1:username}\")",
				"documentation": {
					value: [
						"```lua", "Usage: teleporttoplr(username)", "```",
						"```lua", "Example: teleporttoplr(c00l_username22)", "```",
						"Result: teleport the localplayer's character to the player with \"c00l_username22\" as username",
						"",
						"Teleport the LocalPlayer's character to `username`'s character.",
					].join("\n")
				},
			},

			"plrsendmessage()": {
				"insertText": "plrsendmessage(\"${1:message}\")",
				"documentation": {
					value: [
						"```lua", "Usage: plrsendmessage(\"message to send here\")", "```",
						"```lua", "Example: plrsendmessage(\"Hello guys im gay\")", "```",
						"Result: make your player send \"Hello guys im gay\" in chat",
						"",
						"Make the LocalPlayer send `message` in chat",
					].join("\n")
				},
			},
			...
			"getexecutorversion()": {
				"documentation": {
					value: [
						"Return the executor version as string",
					].join("\n")
				},
			},

			"setplrspeed()": {
				"insertText": "setplrspeed(${1:speed})",
				"documentation": {
					value: [
						"```lua", "Usage: setplrspeed(speed)", "```",
						"```lua", "Example: setplrspeed(180)", "```",
						"Result: set the LocalPlayer Character's WalkSpeed to 180",
						"",
						"Set the LocalPlayer Character's WalkSpeed to 'speed' value",
					].join("\n")
				},
			},

			"set_clipboard()": {
				"insertText": "set_clipboard(${1:data})",
				"documentation": {
					value: [
						"```lua", "Usage: set_clipboard(data)", "```",
						"```lua", "Example: set_clipboard(\"I am NOT gay\")", "```", "Result: set to the user clipboard \"I am NOT gay\"",
						"",
						"Set 'data' to the current user clipboard",
					].join("\n")
				},
			},

			//end onf nitrogengns jdsjf fuck im gettingg angry
